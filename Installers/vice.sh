...
zenity --info --timeout=2 --text="Installing VICE..."
...
if test -f ~/.pocket-home/.version; then
  ...
  then
    jq '(.pages[0] | .items) |= . + [{ "name": "VICE", "icon": "~/PocketInstaller/Icons/vice.png", "shell": "/home/chip/.vice/vicerc" }]' ~/.pocket-home/config.json > tmp.$$.json
    mv tmp.$$.json ~/.pocket-home/config.json
  fi
fi

zenity --info --timeout=2 --text="VICE installed!"
